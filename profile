# Microsoft.PowerShell_profiel.ps1
# from a powershell window after installing vscode, type code $PROFILE to edit it

using namespace System.Management.Automation
using namespace System.Management.Automation.Language


# To make it reset every PowerShell session (not just daily):
$sessionFile = "$env:TEMP\PSSession_$PID.tmp"
...
function Show-WelcomeArt {
    ...
    ╚═╝      ╚═════╝  ╚══╝╚══╝ ╚══════╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝╚══════╝╚══════╝╚══════╝
        ...
        ║                                                                      ║
        ║  💡 Enhanced Profile Loaded - Ready for Maximum Productivity!        ║
        ║  📁 Directory History: cd -l                                         ║
        ║  🔍 Smart History: F7                                                ║
        ║  📋 Paste as Here-String: Ctrl+V                                     ║
        ║  🎯 Quick Directory Marks: Ctrl+Shift+J (mark), Ctrl+J (jump)        ║
        ║                                                                      ║
        ║                    Happy Scripting! ⚡                               ║
...
"@
    ...
    $global:AsciiArtShown = $true
}

# Show ASCII art on first window load
if (-not (Test-Path $sessionFile)) {
    New-Item -Path $sessionFile -ItemType File -Force | Out-Null
    Show-WelcomeArt
    
    # Clean up old session files
    Get-ChildItem "$env:TEMP\PSSession_*.tmp" | 
        Where-Object { $_.Name -notmatch "PSSession_$PID" } | 
        Remove-Item -Force -ErrorAction SilentlyContinue
}

# Enhanced Clear function that hides the ASCII art
function Clear-HostEnhanced {
    param(
        [switch]$ShowArt,
        [switch]$HideArt
    )
    ...
    # If ASCII art was shown and we're not explicitly keeping it, mark as cleared
    if ($global:AsciiArtShown -and -not $HideArt) {
        $global:AsciiArtShown = $false
    }
}

# Custom function to re-show the welcome art
function Show-Welcome {
    Show-WelcomeArt
}

# To add a "nuke" command that removes the session file entirely:
function Reset-WelcomeArt { 
    Remove-Item "$env:TEMP\PSSession_*.tmp" -Force -ErrorAction SilentlyContinue
    $global:AsciiArtShown = $false
    Write-Host "Welcome art reset - will show on next new window" -ForegroundColor Yellow
}

# Override the default clear command
Set-Alias -Name clear -Value Clear-HostEnhanced -Option AllScope -Force
Set-Alias -Name cls -Value Clear-HostEnhanced -Option AllScope -Force

# Additional aliases for convenience
Set-Alias -Name welcome -Value Show-Welcome -Option AllScope
Set-Alias -Name banner -Value Show-Welcome -Option AllScope
Set-Alias -Name nuke-welcome -Value Reset-WelcomeArt -Option AllScope
...
Import-Module Pscx -ArgumentList ~\Pscx.UserPreferences.ps1
Import-Module Pansies
Import-Module ImportExcel

# Initialize Oh My Posh  YOU actually need this for this to take effect.
...
# Unix/Linux Style Aliases for PowerShell Profile
...
# Set-Alias -Name dir -Value Get-ChildItem  # Keep Windows compatibility
Set-Alias -Name tree -Value Get-ChildItemTree
Set-Alias -Name pwd -Value Get-Location

# Custom Functions to Support Unix-like Behavior
...
function Get-ChildItemTree {
    ...
    Format-Table @{Name="Path"; Expression={$_.FullName.Replace((Get-Location).Path, ".")}}, 
